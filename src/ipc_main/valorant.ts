...
import { BrowserWindow, ipcMain, WebRequest } from "electron";
...
let presences: Map<string, ValorantPresence>;
...
export function initialize_valorant_api(browser_window: BrowserWindow) {
  ...
  setInterval(async () => {
    ...
    if (lockfile === prev_lockfile) return;
    if (!lockfile || !logs) return;
    ...
    win.webContents.send("LOCKFILE_UPDATE");
  }, 5000);
}
...
function initialize_presence_monitoring() {
  ...
  query(RequestType.LOCALHOST, "GET", "/chat/v4/presences", true).then(
    (pres) => {
      if (pres && pres.data) {
        presences = process_valorant_presence(
          pres.data["presences"],
          puuid,
          query_game
        );
        win.webContents.send("VALORANT_PRESENCES", presences);
      }
    }
  );
...
function create_chat_listeners() {
  ipcMain.on(
    "VALORANT_CHAT_SEND",
    async (event, cid, message) =>
      (event.returnValue = await query(
        RequestType.LOCALHOST,
        "POST",
        "/chat/v6/messages",
        true,
